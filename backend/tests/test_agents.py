...
class TestResearchState:
    def test_make_initial_state(self) -> None:
        state = make_initial_state(
            query="test query", project_id="proj-1", objective="test objective"
        )
        ...
        with pytest.raises(ValueError):
            AgentRegistry.register(
                type(
                    "NamelessAgent",
                    (BaseAgent,),
                    {"agent_name": "", "arun": lambda self, s: s},
                )
            )
    ...
    def test_register_and_get(self) -> None:
        ...
        agent_cls = type(
            ...
            (BaseAgent,),
            {
                ...
                "__init__": lambda self, llm: (
                    setattr(self, "llm", llm) or setattr(self, "agent_name", "dynamic")
                ),
...
class TestResearchWorkflow:
    ...
    async def test_workflow_with_project_id(self) -> None:
        ...
        result = await workflow.arun(
            query="test", project_id="proj-1", objective="obj-1"
        )
    ...
    async def test_workflow_concurrent_executions(self) -> None:
        ...
        assert all(
            r["status"] in ("completed", "report_generation_complete") for r in results
        )
...
class TestGraphNodes:
    ...
    async def test_summarizer_node(self) -> None:
        ...
        state["retrieved_documents"] = [
            {
                "subtopic": "s1",
                "evidence": [{"content": "doc1 content here", "chunk_id": "c1"}],
                "sources": ["src1"],
            ...
            {
                "subtopic": "s2",
                "evidence": [{"content": "doc2 content here", "chunk_id": "c2"}],
                "sources": ["src2"],
                "confidence_score": 80.0,
                "coverage": True,
            },
