            ...
            if isinstance(refs, list):
                for ref in refs:
                    ...
                    if target_id in graph.nodes:
                        edge = GraphEdge(
                            edge_id=_edge_id(
                                mem_node.node_id, target_id, RelationshipType.REFERENCES
                            ),
    ...
    def build_evidence_relationships(
        ...
        if source_type == "paper" or source_type == "arxiv":
            for pid, pnode in paper_nodes.items():
                edge = GraphEdge(
                    edge_id=_edge_id(
                        evidence_node.node_id, pid, RelationshipType.REFERENCES
                    ),
                    source_id=evidence_node.node_id,
                    target_id=pid,
                    relationship_type=RelationshipType.REFERENCES,
                    label="Evidence references paper",
                    metadata=EdgeMetadata(
        ...
        for mid, mnode in method_nodes.items():
            if mid in evidence_node.description or mid in str(props):
                edge = GraphEdge(
                    ...
                    label="Evidence mentions method",
                    metadata=EdgeMetadata(
                        confidence=0.5,
                        weight=0.5,
    ...
    def build_summary_relationships(
        ...
        for fn in finding_nodes:
            edges.append(
                GraphEdge(
                    edge_id=_edge_id(
                        summary_node.node_id, fn.node_id, RelationshipType.CONTAINS
                    ),
        ...
        for gn in gap_nodes:
            edges.append(
                GraphEdge(
                    edge_id=_edge_id(
                        summary_node.node_id, gn.node_id, RelationshipType.IDENTIFIES
                    ),
                    source_id=summary_node.node_id,
        ...
        for fn in finding_nodes:
            for en in evidence_nodes:
                edges.append(
                    GraphEdge(
                        edge_id=_edge_id(
                            fn.node_id, en.node_id, RelationshipType.REFERENCES
                        ),
    ...
    def build_analysis_relationships(
        ...
        for cn in consensus_nodes:
            edges.append(
                GraphEdge(
                    edge_id=_edge_id(
                        analysis_node.node_id, cn.node_id, RelationshipType.CONTAINS
                    ),
                    source_id=analysis_node.node_id,
        ...
        for tn in trend_nodes:
            edges.append(
                GraphEdge(
                    edge_id=_edge_id(
                        analysis_node.node_id, tn.node_id, RelationshipType.IDENTIFIES
                    ),
                    source_id=analysis_node.node_id,
        ...
        for ln in limitation_nodes:
            edges.append(
                GraphEdge(
                    edge_id=_edge_id(
                        analysis_node.node_id, ln.node_id, RelationshipType.IDENTIFIES
                    ),
                    source_id=analysis_node.node_id,
                    target_id=ln.node_id,
                    relationship_type=RelationshipType.IDENTIFIES,
        ...
        for rn in recommendation_nodes:
            edges.append(
                GraphEdge(
                    edge_id=_edge_id(
                        analysis_node.node_id, rn.node_id, RelationshipType.GENERATES
                    ),
                    source_id=analysis_node.node_id,
                    target_id=rn.node_id,
        ...
        for rn in recommendation_nodes:
            for fn in finding_nodes:
                edges.append(
                    GraphEdge(
                        edge_id=_edge_id(
                            rn.node_id, fn.node_id, RelationshipType.DERIVED_FROM
                        ),
    ...
    def build_methodology_relationships(
        ...
        for mn in method_nodes:
            edges.append(
                GraphEdge(
                    edge_id=_edge_id(
                        methodology_node.node_id, mn.node_id, RelationshipType.CONTAINS
                    ),
                    source_id=methodology_node.node_id,
                    target_id=mn.node_id,
                    relationship_type=RelationshipType.CONTAINS,
                    label="Methodology contains method",
                    metadata=EdgeMetadata(
                        confidence=0.9,
                        weight=0.8,
                        source="methodology_structure",
            ...
            for dn in dataset_nodes:
                edges.append(
                    GraphEdge(
                        ...
                        source_id=mn.node_id,
                        target_id=dn.node_id,
                        relationship_type=RelationshipType.USES,
                        label="Method uses dataset",
                        metadata=EdgeMetadata(
                            confidence=0.7,
                            weight=0.6,
                            source="methodology_inference",
        ...
        for bn in benchmark_nodes:
            edges.append(
                GraphEdge(
                    edge_id=_edge_id(
                        methodology_node.node_id, bn.node_id, RelationshipType.CONTAINS
                    ),
                    source_id=methodology_node.node_id,
                    target_id=bn.node_id,
                    relationship_type=RelationshipType.CONTAINS,
            ...
            for mn in method_nodes:
                edges.append(
                    GraphEdge(
                        edge_id=_edge_id(
                            bn.node_id, mn.node_id, RelationshipType.EVALUATES
                        ),
                        source_id=bn.node_id,
                        target_id=mn.node_id,
                        relationship_type=RelationshipType.EVALUATES,
                        label="Benchmark evaluates method",
                        metadata=EdgeMetadata(
                            confidence=0.6,
                            weight=0.5,
                            source="methodology_inference",
        ...
        for rn in risk_nodes:
            edges.append(
                GraphEdge(
                    edge_id=_edge_id(
                        methodology_node.node_id,
                        rn.node_id,
                        RelationshipType.IDENTIFIES,
                    ),
                    source_id=methodology_node.node_id,
                    target_id=rn.node_id,
                    relationship_type=RelationshipType.IDENTIFIES,
        ...
        for dn in dataset_nodes:
            edges.append(
                GraphEdge(
                    edge_id=_edge_id(
                        methodology_node.node_id, dn.node_id, RelationshipType.CONTAINS
                    ),
                    source_id=methodology_node.node_id,
                    target_id=dn.node_id,
                    relationship_type=RelationshipType.CONTAINS,
    ...
    def build_experiment_relationships(
        ...
        for hn in hypothesis_nodes:
            edges.append(
                GraphEdge(
                    edge_id=_edge_id(
                        experiment_node.node_id, hn.node_id, RelationshipType.CONTAINS
                    ),
                    source_id=experiment_node.node_id,
                    target_id=hn.node_id,
                    relationship_type=RelationshipType.CONTAINS,
                    label="Experiment contains hypothesis",
                    metadata=EdgeMetadata(
                        confidence=0.9,
                        weight=0.8,
                        source="experiment_structure",
        ...
        for vn in variable_nodes:
            edges.append(
                GraphEdge(
                    edge_id=_edge_id(
                        experiment_node.node_id, vn.node_id, RelationshipType.CONTAINS
                    ),
                    source_id=experiment_node.node_id,
                    target_id=vn.node_id,
                    relationship_type=RelationshipType.CONTAINS,
        ...
        for rn in risk_nodes:
            edges.append(
                GraphEdge(
                    edge_id=_edge_id(
                        experiment_node.node_id, rn.node_id, RelationshipType.IDENTIFIES
                    ),
                    source_id=experiment_node.node_id,
                    target_id=rn.node_id,
                    relationship_type=RelationshipType.IDENTIFIES,
        ...
        for pn in phase_nodes:
            edges.append(
                GraphEdge(
                    edge_id=_edge_id(
                        experiment_node.node_id, pn.node_id, RelationshipType.CONTAINS
                    ),
                    source_id=experiment_node.node_id,
                    target_id=pn.node_id,
                    relationship_type=RelationshipType.CONTAINS,
            ...
            for sn in step_nodes:
                if sn.metadata.properties.get("parent_phase") == pn.node_id:
                    edges.append(
                        GraphEdge(
                            edge_id=_edge_id(
                                pn.node_id, sn.node_id, RelationshipType.CONTAINS
                            ),
                            source_id=pn.node_id,
        ...
        for on in objective_nodes:
            edges.append(
                GraphEdge(
                    edge_id=_edge_id(
                        experiment_node.node_id, on.node_id, RelationshipType.CONTAINS
                    ),
                    source_id=experiment_node.node_id,
                    target_id=on.node_id,
                    relationship_type=RelationshipType.CONTAINS,
        ...
        for ocn in outcome_nodes:
            edges.append(
                GraphEdge(
                    edge_id=_edge_id(
                        experiment_node.node_id, ocn.node_id, RelationshipType.LEADS_TO
                    ),
                    source_id=experiment_node.node_id,
                    target_id=ocn.node_id,
        ...
        for hn in hypothesis_nodes:
            for mn in method_nodes:
                edges.append(
                    GraphEdge(
                        edge_id=_edge_id(
                            hn.node_id, mn.node_id, RelationshipType.VALIDATES
                        ),
                        source_id=hn.node_id,
                        target_id=mn.node_id,
        ...
        for dn in dataset_nodes:
            edges.append(
                GraphEdge(
                    edge_id=_edge_id(
                        experiment_node.node_id, dn.node_id, RelationshipType.USES
                    ),
                    source_id=experiment_node.node_id,
                    target_id=dn.node_id,
                    relationship_type=RelationshipType.USES,
    ...
    def build_domain_relationships(
        ...
        for dn in dataset_nodes:
            edges.append(
                GraphEdge(
                    edge_id=_edge_id(
                        dn.node_id, domain_node.node_id, RelationshipType.BELONGS_TO
                    ),
                    source_id=dn.node_id,
        ...
        for mn in method_nodes:
            edges.append(
                GraphEdge(
                    edge_id=_edge_id(
                        mn.node_id, domain_node.node_id, RelationshipType.BELONGS_TO
                    ),
                    source_id=mn.node_id,
                    target_id=domain_node.node_id,
                    relationship_type=RelationshipType.BELONGS_TO,
    ...
    def build_author_paper_relationships(
        ...
        for pn in paper_nodes:
            edges.append(
                GraphEdge(
                    edge_id=_edge_id(
                        author_node.node_id, pn.node_id, RelationshipType.WROTE
                    ),
                    source_id=author_node.node_id,
                    target_id=pn.node_id,
                    relationship_type=RelationshipType.WROTE,
                    label="Author wrote paper",
                    metadata=EdgeMetadata(
                        confidence=0.9,
                        weight=0.9,
    ...
    def build_sequential_relationships(
        ...
        return GraphEdge(
            ...
            metadata=EdgeMetadata(
                ...
                reason=reason
                or f"Relationship between {node_a.node_type.value} and {node_b.node_type.value}",
            ),
        )
