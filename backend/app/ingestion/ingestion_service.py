...
class IngestionService:
    ...
    Upload → Validate → Extract text → Chunk → Embed → Store vectors → Record metadata
    """

    def __init__(self, db: AsyncSession) -> None:
        self.db = db
        settings = get_settings()
        self.upload_validator = UploadValidator(settings)
    ...
    async def ingest_document(
        ...
        try:
            self.upload_validator.validate_file(file_content, filename)
        except Exception as e:
            logger.warning(
                "File validation failed", extra={"error": str(e), "filename": filename}
            )
            raise
        ...
        base_meta = extract_default_metadata(
            ...
            if isinstance(collection, CollectionName)
            else collection,
            filename=filename,
            author=author,
            **(metadata_extra or {}),
