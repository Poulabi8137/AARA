...
class PlannerAgent(BaseAgent):
    ...
    async def arun(self, state: ResearchState) -> ResearchState:
            ...
            try:
                ...
                if plan_data is None:
                    if attempt > 0:
                        repair_attempts += 1
                    logger.warning(
                        "planner parse failed", extra={"attempt": attempt + 1}
                    )
                    continue
                ...
                if warnings:
                    ...
                    logger.info(
                        "planner validation warnings", extra={"warnings": warnings}
                    )
        ...
        fallback["research_questions"] = [
            q.format(query=query) for q in fallback["research_questions"]
        ]
        fallback["keywords"] = [k.format(query=query) for k in fallback["keywords"]]
        fallback["search_queries"] = [
            sq.format(query=query) for sq in fallback["search_queries"]
        ]
        fallback["subtopics"] = [st.format(query=query) for st in fallback["subtopics"]]
        fallback["expected_deliverables"] = [
            d.format(query=query) for d in fallback["expected_deliverables"]
        ]
        fallback["priority_areas"] = [
            p.format(query=query) for p in fallback["priority_areas"]
        ]
        ...
        def _safe_plan(data: dict) -> PlannerOutput:
            ...
            rq = [
                r for r in data.get("research_questions", []) if r.strip().endswith("?")
            ]
            while len(rq) < 3:
                rq.append(f"What is area {len(rq) + 1} of {query}?")
            return PlannerOutput(
                research_goal=data.get("research_goal", f"Research on {query}")
                or f"Research on {query}",
                research_questions=rq,
                keywords=kw,
                search_queries=sq,
                subtopics=st,
                methodology=data.get("methodology", "literature review")
                or "literature review",
                expected_deliverables=[
                    d
                    for d in data.get("expected_deliverables", [f"Report on {query}"])
                    if d.strip()
                ]
                or [f"Report on {query}"],
                priority_areas=[
                    p
                    for p in data.get("priority_areas", [f"Understanding {query}"])
                    if p.strip()
                ]
                or [f"Understanding {query}"],
        ...
        logger.info(
            "planner completed with fallback",
            extra={
                "query": query[:80],
                "planning_score": plan.planning_score,
            },
